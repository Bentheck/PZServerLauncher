    ...
    AuditStore auditStore)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var availableBranches = await steamCmdToolService.GetBranchCatalogAsync(
            profile.InstallDirectory,
            profile.SteamBranch,
            cancellationToken: cancellationToken);
        var selectedBranch = availableBranches.Branches.FirstOrDefault(branch =>
            string.Equals(branch.Name, steamBranch, StringComparison.OrdinalIgnoreCase));
        if (selectedBranch is null)
        {
            throw new InvalidOperationException($"Steam branch '{steamBranch}' is no longer advertised for this dedicated server. Refresh versions and choose an available branch.");
        }
        ...
        var scriptPlan = planner.CreateInstallScript(profile, selectedBranch.Name);
